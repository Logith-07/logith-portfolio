...
function getStaticChatResponse(message) {
    ...
    if (chatIncludes(raw, ['your name']) || raw === 'name') {
        return { text: 'Logith S T.', options: [] };
    }
    ...
    if (chatHasWords(raw, ['project'])) {
        return { text: 'Projects: Vip store; Portfolio Website; Southern Species; Cine Book.', options: [] };
    }
    ...
    if (chatHasWords(raw, ['experience']) || chatHasWords(raw, ['internship'])) {
        return { text: 'Experience: Web development.', options: [] };
    }
    if (chatHasWords(raw, ['contact'])) {
        return {
            text: 'Contact: GitHub: https://github.com/Logith-07 | LinkedIn: https://www.linkedin.com/in/logith-s-t-0476ba2a7 | Portfolio: https://l-portfolio-jmy3.vercel.app//',
            options: []
        };
    }
...
async function sendMessage(optMessage) {
    ...
    if (!optMessage && chatInput) chatInput.value = '';
        ...
        try {
            const apiUrl = 'https://logith-07.github.io/chatbot/';
